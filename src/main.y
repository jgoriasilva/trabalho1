...
Código: main.y
Objetivo: Implementar a gramática responsável por compreender expressões matemáticas
...
S:
	...
	| S C EOL { printf("\n; Insira uma expresão ou comando: "); }
	|
	;
...
C:
 	VAR '=' exp1 {	char name = $<rotulo>1; /* Recebe o nome da variável e atribui à variável 'name' */
			...
			if(!flag[name-'a']) { /* Verifica em qual caso (explicado anteriormente) tal variável se encaixa, a partir do vetor 'flag'. */
				printf("\tJMP jump%c\nv%c:\njump%c:\n\tMOV C, v%c\n\tPOP A\n\tMOV [C], A", name, name, name, name); /* Aqui, é implementada a primeira alocação da variável. Para isso, primeiro dá-se o comando JMP jump'name', para que o código prossiga com a sua execução. Depois, aloca-se um espaço de memória para a variável v'name'. Cria-se então o rótulo jump'name' que é exatamente para onde o código pula com o comando JMP jump'name'. Depois, busca-se onde na mémoria foi alocada a variável v'name' e atribui seu endereço no registrador C. Então, o valor desejado para a atribuição é retirado da pilha e finalmente armazenado no endereço apontado por C. Assim, a variável tem um espaço de memória reservado e seu valor está atribuído. */
				flag[name-'a'] = 1; } /* Atualiza-se o valor da flag de tal variável para que ela passe a cair sempre no segundo caso. */
			else
				printf("\tMOV C, v%c\n\tPOP A\n\tMOV [C], A", name); } /* Caso a variável caia no segundo caso, simplesmente pega-se o endereço reservado para a variável, coloca-lhe no registrador C e então move-se o valor desejado de atribuição para o endereço apontado por C. */
	;
...
exp1:
	exp1 '+' exp2 {	$<valor>$ = $<valor>1 + $<valor>3; 
			printf("\tPOP A\n\tPOP B\n\tADD A, B\n\tPUSH A\n"); } /* Para resolução da expressão, simplesmente puxa-se da pilha o valor de ambas as expressões para os registradores A e B e então soma-se os valores de A e B e armazena o resultado no registrador A. O valor de A é então subido para a pilha.*/
	| exp2 { $<valor>$ = $<valor>1; }
	;
...
exp2:
	...
	/* A potenciação é resolvida primeiramente puxando-se os valores de 'exp3' e 'exp4' para os registradores C e B, respectivamente. Então, verifica-se se B (o expoente) é igual a 0. Se for o caso, o programa pula para o rótulo .zero'count' (o count é um contador que incrementa a cada operação de exponenciação para que o código não retorne a loops errados). Neste rótulo, o registrador A recebe o valor 1 e então o código segue para o rótulo .end'count'. Isto ocorre porque qualquer número elevado a 0 deve retornar 1 como resultado. Caso o expoente não seja 0, então o código segue para o rótulo .nzero'count'. Lá, é implementado um loop (com rótulo .loop'count') para que B seja decrementado e A seja multiplicado sucessivamente por C até que B seja igual a 1. Isto implementa exatamente a operação de potenciação. Depois, o código segue naturalmente para o rótulo .end'count' onde o resultado armazenado em A é "empurrado" para a pilha. */
...
int main() {
	...
	printf("; COPIE A PARTIR DAQUI PARA SIMULAÇÃO\n");
	printf("; Inicialização dos registradores");
	printf("\n\tMOV A, 0\n\tMOV B, 0\n\tMOV C, 0\n\tMOV D, 0\n");
	printf("; Insira uma expressão ou comando: ");
  	yyparse();
    	return 0;
}
